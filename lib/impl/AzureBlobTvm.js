...
const logger = require('@adobe/aio-lib-core-logging')('@adobe/aio-tvm:AzureBlobTvm')
...
class AzureBlobTvm extends Tvm {
  ...
  async _generateCredentials (params) {
    ...
    logger.warn('_generateCredentials')
    ...
    let sasQueryParamsPrivate, sasQueryParamsPublic
    try {
      sasQueryParamsPrivate = azure.generateBlobSASQueryParameters({ ...commonSasParams, containerName: privateContainerName }, sharedKeyCredential)
      sasQueryParamsPublic = azure.generateBlobSASQueryParameters({ ...commonSasParams, containerName: publicContainerName }, sharedKeyCredential)
    } catch (e) {
      console.error('Error generating SAS token', e)
    }
    return {
