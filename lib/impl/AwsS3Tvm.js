...
class AwsS3Tvm extends Tvm {
  ...
  constructor () {
    ...
    this._addToValidationSchema('s3TagKey')
    this._addToValidationSchema('s3TagValue')
    this._addToValidationSchema('region')
...
/**
 * Check and create new s3 bucket if it doesnt exists
 *
...
async function checkAndCreateS3Bucket (params) {
  ...
  try {
    await s3.headBucket(args).promise()
  } catch (err) {
    if (err.statusCode === 404) {
      logger.info('Trying to create bucket - ' + bucketName)
      await s3.createBucket(args).promise()
        .catch(e => { throw new Error(`AWS S3 Error: ${err.message}`) })

      logger.info('Bucket created - ' + bucketName)
      await addTagToBucket(s3, bucketName, 'test', 'foo')

      await addTagToBucket(s3, bucketName, params.s3TagKey, params.s3TagValue)
      logger.info('Added Tag to bucket - ' + bucketName)
    } else { throw new Error(`AWS S3 Error: ${err.message}`) }
  }
  return bucketName
}
...
function getBucketName (namespace, prefix) {
  return prefix + '-' + crypto.createHash('sha256').update(namespace).digest('hex').slice(0, 32)
}
