...
class AzurePresignTvm extends Tvm {
  ...
  async _generateCredentials (params) {
    ...
    const privateContainerURL = azure.ContainerURL.fromServiceURL(serviceURL, containerName)

    const identifier = azureUtil.getAccessPolicy(privateContainerURL, params.azureStorageAccount, params.azureStorageAccessKey)
