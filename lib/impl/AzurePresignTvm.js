...
class AzurePresignTvm extends Tvm {
  ...
  constructor () {
    ...
    this._addToValidationSchema('expiryInSeconds', joi.number().integer().min(2).max(86400).required())
  }
  ...
  async _generateCredentials (params) {
    ...
    // generate SAS token
    const expiryTime = new Date(new Date().getTime() + (1000 * params.expiryInSeconds))
    const permissions = azure.BlobSASPermissions.parse('r')
