...
class AzureCosmosTVM extends TVM {
  ...
  async _generateCredentials (params) {
    ...
    const resourceToken = (await user.permissions.create({ id: permissionId, permissionMode: cosmos.PermissionMode.All, resource: container.url, resourcePartitionKey: [hexName] }, { resourceTokenExpirySeconds: params.expirationDuration })).resource._token
