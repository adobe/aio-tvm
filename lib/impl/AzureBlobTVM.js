...
class AzureBlobTVM extends TVM {
  ...
  async _generateCredentials (params) {
    ...
    try {
      await azure.ContainerURL.fromServiceURL(serviceURL, publicContainerName).create(azure.Aborter.none, { access: 'blob', metadata: { namespace: params.owNamespace } })
      await azure.ContainerURL.fromServiceURL(serviceURL, privateContainerName).create(azure.Aborter.none, { metadata: { namespace: params.owNamespace } })
      ...
      // console.log(`Did not create containers: ${privateContainerName}, ${publicContainerName} already exist`)
    }
