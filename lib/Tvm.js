...
class Tvm {
  ...
  constructor () {
    this._validationSchema = {
      ...
      disableMetrics: joi.boolean().optional()
    }
  }
  ...
  async processRequest (params) {
    // we depend on a metrics-url, otherwise we disable metrics
    if (params['metrics-url']) {
      metricsEnabled = true
      setMetricsURL(params['metrics-url'])
    }
    ...
    try {
      ...
      try {
        ...
        if (metricsEnabled) {
          incBatchCounter('error_count', params.owNamespace, '403')
        }
