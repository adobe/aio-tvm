...
const { setMetricsURL, incBatchCounter } = require('aio-lib-metrics-beta')
...
class Tvm {
  ...
  async processRequest (params) {
    ...
    try {
      ...
      // set metrics url after validating parameters, metricsUrl has to be a valid url
      metricsEnabled = !!params.metricsUrl
      if (metricsEnabled) {
      ...
      if (params.disableAdobeIOApiGwTokenValidation !== 'true') {
        try {
          ...
          incBatchCounter('warning_count', params.owNamespace, '403-apigw')
          return Tvm._userErrorResponse('unauthorized', 403)
        }
      }
      ...
      try {
        ...
        if (metricsEnabled) {
          incBatchCounter('request_count', params.owNamespace)
        }
      } catch (e) {
        ...
        if (metricsEnabled) {
          incBatchCounter('warning_count', params.owNamespace, '403-auth')
        }
      ...
      logger.info(`credentials generated - request Id - ${requestId}`)
      ...
      logger.error(e)
      if (metricsEnabled) {
        incBatchCounter('error_count', e.message, '500')
      }
