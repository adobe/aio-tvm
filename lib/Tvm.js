...
class Tvm {
  ...
  async _validateAdobeIOApiGwServiceToken (params) {
    ...
    if (!validApiGWTokensCache.get(token)) {
      ...
      validApiGWTokensCache.set(token, true)
    }
  }
    ...
    try {
      ...
      if (params.disableAdobeIOApiGwTokenValidation !== 'true') {
        try {
          ...
          logger.warn(`Adobe I/O API Gateway service token is not valid: ${e.message}`)
          return Tvm._userErrorResponse('Adobe I/O API Gateway service token is not valid', 403)
        }
      }
