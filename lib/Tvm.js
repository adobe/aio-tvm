...
class Tvm {
  ...
  async _validateAdobeIOApiGwServiceToken (params) {
    ...
    // attempt to extract the bearer token from the header
    const token = (params.__ow_headers[REQUIRED_HEADER] || '').split('Bearer ')[1]
    if (!token) {
    ...
    if (!res.valid) {
      throw new Error('bad api gateway service token: is not valid')
    }
    // 2. check client id
    ...
    if (missing.length > 0) {
      throw new Error(`bad api gateway service token: missing required scope ${missing.toString()}`)
    }
