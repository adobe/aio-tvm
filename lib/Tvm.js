...
let lastValidApiGwServiceToken = null
...
class Tvm {
  ...
  async _validateAdobeIOApiGwServiceToken (params) {
    const token = (params.__ow_headers['x-gw-ims-authorization'] || '').split('Bearer ')[1]

    if (!token) {
      throw new Error('bad api gateway service token: missing or not a bearer token')
    }

    const ims = new Ims(params.imsEnv || 'prod')
    // todo offline validation or caching (reuse caching logic from validators)
    // for covering non valid tokens
    if (token !== lastValidApiGwServiceToken) {
      const res = await ims.validateToken(params.__ow_headers.ims)
      if (!res.valid) {
        throw new Error('bad api gateway service token: is not a valid')
      }
      if (res.token.client_id !== 'AnsApiPlatform1') {
        throw new Error(`bad api gateway service token: not allowed client_id ${res.client_id}`)
      }
      const scopes = res.token.scopes.split(',')
      if (!scopes.includes('openid') || !scopes.includes('AdobeID') || !scopes.includes('system')) {
        throw new Error('bad api gateway service token: missing required scopes')
      }
      lastValidApiGwServiceToken = token
    }
  }
