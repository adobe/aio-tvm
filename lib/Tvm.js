...
class Tvm {
  ...
  constructor () {
    ...
    const tempSchema = {
      presignReq: joi.boolean().required(),
      blobName: joi.string().min(3).required(),
      expiryInSeconds: joi.number().integer().min(30).max(18000).required()
    }

    this._presignReqValidationSchema = Object.assign(tempSchema, this._validationSchema)
  }
  ...
  _addToValidationSchema (key, type) {
    ...
    this._presignReqValidationSchema[key] = type || joi.string().required()
  }
  ...
  _validateRequestParams (params) {
    // use schema based on type of request
    const schema = params.presignReq ? this._presignReqValidationSchema : this._validationSchema
    return joi.object().label('params').keys(schema)
      .required()
   ...
   * @memberof Tvm
   */
  async _generatePresignCredentials (params) {
    throw new Error('not implemented')
  }
  ...
  async processRequest (params) {
    try {
      ...
      let credentials
      if (params.presignReq) {
        credentials = await this._generatePresignCredentials(params)
        logger.info('presign credentials generated')
      } else {
        credentials = await this._generateCredentials(params)
      ...
      return {
        error: {
          body: { error: 'server error ' + e.message },
          statusCode: 500
        }
      }
    }
