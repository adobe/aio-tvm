...
class Tvm {
  ...
  async _validateAdobeIOApiGwServiceToken (params) {
    ...
    const tokenHeader = params.__ow_headers[REQUIRED_HEADER]
    if (!tokenHeader) {
      throw new Error('missing x-gw-ims-authorization header')
    }
    const token = tokenHeader.split('Bearer ')[1]
    if (!token) {
      throw new Error('x-gw-ims-authorization header is not a valid Bearer token')
    }
